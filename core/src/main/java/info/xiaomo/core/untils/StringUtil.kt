...
class StringUtil : StringUtils() {
    companion object {
         ...
         * @return 转换后的字符串
         */
        fun replace(s: String, map: Map<String, String>): String {
            val sb = StringBuilder((s.length * 1.5).toInt())
            var cursor = 0
            val s1 = "\${"
            val s2 = '}'
            var start: Int
            var end: Int
            while ((start = s.indexOf(s1, cursor)) != -1 && (end = s.indexOf(s2.toInt(), start)) != -1) {
                sb.append(s.substring(cursor, start))
                val key = s.substring(start + 2, end)
                sb.append(map[StringUtils.trim(key)])
                cursor = end + 1
            }
        ...
        fun getIP(request: HttpServletRequest): String? {
            val requested = "X-Requested-For"
            var ip = request.getHeader(requested)
            val unknown = "unknown"
            if (StringUtils.isBlank(ip) || unknown.equals(ip, ignoreCase = true)) {
                val forwarded = "X-Forwarded-For"
                ip = request.getHeader(forwarded)
            }
            if (StringUtils.isBlank(ip) || unknown.equals(ip, ignoreCase = true)) {
                val client = "Proxy-Client-IP"
                ip = request.getHeader(client)
            }
            if (StringUtils.isBlank(ip) || unknown.equals(ip, ignoreCase = true)) {
                val wl = "WL-Proxy-Client-IP"
                ip = request.getHeader(wl)
            }
            if (StringUtils.isBlank(ip) || unknown.equals(ip, ignoreCase = true)) {
                val http = "HTTP_CLIENT_IP"
                ip = request.getHeader(http)
            }
            if (StringUtils.isBlank(ip) || unknown.equals(ip, ignoreCase = true)) {
                val httpx = "HTTP_X_FORWARDED_FOR"
                ip = request.getHeader(httpx)
            }
            if (StringUtils.isBlank(ip) || unknown.equals(ip, ignoreCase = true)) {
        ...
        fun isocode(tempSql: String): String {
            ...
            try {
                ...
                returnString = String(ascii, "ISO-8859-1")
            } catch (e: Exception) {
                e.printStackTrace()
            }
         ...
         * @return
         */
        fun convertCode(srcCode: String, destCode: String, strTmp: String): String {
            var returnString = convertNullCode(strTmp)
            try {
                val ascii = returnString.toByteArray(charset(srcCode))
                returnString = String(ascii, destCode)
            } catch (e: Exception) {
                e.printStackTrace()
            }
        ...
        fun gbk2big5code(tempSql: String): String {
            ...
            try {
                val ascii = returnString.toByteArray(charset("GBK"))
                returnString = String(ascii, "big5")
            } catch (e: Exception) {
                e.printStackTrace()
            }
        ...
        override fun replace(strSrc: String?, strOld: String?, strNew: String?): String {
        ...
        fun toChinese(strVal: String?): String {
            ...
            try {
                if (strVal == null) {
                    ...
                    strVal = String(strVal.toByteArray(charset("ISO8859_1")), "GBK")
                    return strVal
                }
            } catch (exp: Exception) {
                return ""
        ...
        fun toGBK(strVal: String?): String {
            ...
            try {
                if (strVal == null) {
                    ...
                    strVal = strVal.trim { it <= ' ' }
                    strVal = String(strVal.toByteArray(charset("UTF-8")), "GBK")
                    return strVal
                }
            } catch (exp: Exception) {
                return ""
        ...
        fun toISO(strVal: String?): String {
            ...
            try {
                if (strVal == null) {
                    ...
                    strVal = String(strVal.toByteArray(charset("GBK")), "ISO8859_1")
                    return strVal
                }
            } catch (exp: Exception) {
                return ""
        ...
        fun gbk2UTF8(strVal: String?): String {
            ...
            try {
                if (strVal == null) {
                    ...
                    strVal = String(strVal.toByteArray(charset("GBK")), "UTF-8")
                    return strVal
                }
            } catch (exp: Exception) {
                return ""
        ...
        fun iso2utf8(strVal: String?): String {
            ...
            try {
                if (strVal == null) {
                    ...
                    strVal = String(strVal.toByteArray(charset("ISO-8859-1")), "UTF-8")
                    return strVal
                }
            } catch (exp: Exception) {
                return ""
        ...
        fun utf82iso(strVal: String?): String {
            ...
            try {
                if (strVal == null) {
                    ...
                    strVal = String(strVal.toByteArray(charset("UTF-8")), "ISO-8859-1")
                    return strVal
                }
            } catch (exp: Exception) {
                return ""
        ...
        fun stringToUnicode(text: String): String {
            ...
            try {
                isr = StringReader(String(text.toByteArray(), "GBK"))
        ...
        fun decode(s: String): String {
            ...
            while (i < len) {
                ...
                if ('A' <= ch && ch <= 'Z') {
                    ...
                    var cint = 0
                    if ('u' != s[i + 1]) {
                        cint = cint shl 4 or VAL[s[i + 1]]
                        cint = cint shl 4 or VAL[s[i + 2]]
                        i += 2
                    } else {
                        cint = cint shl 4 or VAL[s[i + 2]]
                        cint = cint shl 4 or VAL[s[i + 3]]
                        cint = cint shl 4 or VAL[s[i + 4]]
                        cint = cint shl 4 or VAL[s[i + 5]]
                        i += 5
                    }
